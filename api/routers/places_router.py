...
async def create_place(request: Request, place_in: place_model.PlaceBase) -> JSONResponse:
    place_in = jsonable_encoder(place_in)
    new_place = await PlaceService.create(request, place_in)
    return JSONResponse(status_code=status.HTTP_201_CREATED, content={"message" : "Place added successfully", "data" : new_place})
...
async def get_all_places(request: Request) -> JSONResponse:
    places_information = await PlaceService.get_all_places(request)
    return JSONResponse(status_code=status.HTTP_200_OK, content={"message" : "List retrieved successfully", "data": places_information})
...
async def get_place(request: Request, id: str = Path(..., title="The ID of the place to get")) -> JSONResponse:
    place_information = await PlaceService.get_place_by_id(request, id)
    return JSONResponse(status_code=status.HTTP_200_OK, content={"message" : "Place retrieved successfully", "data": place_information})
